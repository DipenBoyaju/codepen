...
const HomePage = () => {
  ...
  return (
    <div className="bg-[#131417] p-8 overflow-hidden">
      <div className="grid md:grid-cols-2">
        <div className="py-12">
          <div className="flex gap-5">
            ...
            <h1 className="text-[2.6rem] leading-tight font-bold text-zinc-50">The best place to build, test, and discover front-end code.</h1>
          </div>
          <p className="text-lg font-lato text-zinc-300 py-5">CodePen is a <span className="font-bold">social development environment</span> for front-end designers and developers. Build and deploy a website, show off your work, build test cases to learn and debug, and find inspiration.</p>
          <button className="font-lato bg-primary p-3 px-4 font-light rounded-md mt-2" onClick={() => nav('/signup')} >Sign Up for Free</button>
        </div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-14 md:gap-7 py-20 relative">
        <img src={curvebg} alt="" className="absolute -right-32 w-[39rem] -top-20" />
        {
          InfoData.data.map((item, index) => (
            <InfoCard item={item} key={index} />
          ))
        }
        <img src={linebg} alt="" className="absolute bottom-6 -left-32 z-0 h-16" />
      </div>
      <ProjectView />
    </div>
  )
